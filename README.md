...
- `r1_main`
  - 操縦や自動動作の流れを決める司令塔です。
- `r1_control`
  - 移動制御や経路追従など、制御理論寄りの処理を担当します。
- `r1_machine`
  - モータや GPIO など実機ハードウェアとの接続を担当します。
- `r1_msgs`
  - このロボット専用の ROS メッセージ定義です。
- `r1_util`
  - 共通で使う補助関数群です。
...
1. オペレータ入力や自動動作の状態遷移を `r1_main_node` が管理します。
...
つまり、`r1_main` が「何をしたいか」を決め、`r1_control` と `r1_machine` が「どう動かすか」を担当する構成です。
...
手動操縦、自動動作、状態遷移、初期化、Sabacan reset のきっかけ作りなどを行います。
...
初見の人は、「このロボットが今どんな手順で動くか」はまず `r1_main_node` を見ると把握しやすいです。
...
モータ、エンコーダ、GPIO、足回り、オドメトリなどの「ハードウェアに近い層」をまとめています。

主なプログラム:

- `r1_machine_manage_node`
  - `r1_msgs` の機構用 topic と Sabacan 用 topic の橋渡しを行います。
  - 非常停止の監視や、`/r1_machine_initialize` による復帰処理も担当します。
...
この package は、`r1_main` が出した「こう動け」という指令を、実際にモータへ流せる形へ落とし込む役目です。
...
複数 package 間で同じ意味の topic を安全に扱うために必要です。
...
ロボットの動作に使う補助データ置き場です。  
経路データや waypoint など、ソースコードではない実行用データを置きます。
...
- 一部 docs は古い可能性があります。
- recent refactor に追従していない説明が残っている可能性があるので、最終的にはソースコードと launch 設定もあわせて確認してください。

## 外部共通ライブラリについて

`bno086` や `sabacan` など、複数機体で共有しているライブラリは `gakurobo2026_common` 側にあります。  
この repo は、R1 固有の構成とロジックを主に管理する場所です。
