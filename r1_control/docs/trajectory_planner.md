...
`zone` は `"blue"` / `"red"` をパラメータファイルへ記録します。waypoint はゾーン別 CSV として扱い、**blue のときも x 座標や theta は反転しません**。

内部では:

- waypoint は読み込んだ CSV の座標系のまま保持
- 軌道計算・描画・保存でも `zone` による `x` / `theta` 変換は行わない

という方針です。青ゾーン用に調整済みの waypoint は、青ゾーン用のファイルベースへそのまま保存します。
...
- waypoint と軌道はゾーン別 CSV の座標をそのまま描画します。
...
4. **フィールド & 軌道表示領域**
   ...
   - マウスクリック:  
     ...
     - `zone == "blue"` の表示中でも、クリックした座標を反転せずそのまま保存します。
