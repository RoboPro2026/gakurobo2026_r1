...
`TrajectoryFollower` は waypoint 探索半径 `search_radius` に基づいて現在追従すべき点を進め、通常時と終点近傍で別の PID ゲインを使います。終点で位置・姿勢誤差が閾値内に入り、その状態が `finish_time_threshold` 秒以上継続したときに追従完了と判定します。
