...
`/robot_move` を受け取ったときは、対象 ACT を軌道インデックスへ変換し、`forest_order` に含まれる番号に応じて軌道上の `v_trans` を `decel_speed` 以下へ落とす区間を上書きします。その後に軌道を再生成し、`act_step_` を更新します。現行実装では、内回りの減速判定は `ACT2`、外回りは `ACT3` に対して有効です。`kfs_mechanism_type` も未使用ではなく、`collect_kfs_offset` を適用する向きを決めるために参照します。
...
起動時に、実装で確保されている軌道本数ぶんのファイルを読み込みます。現行実装では `0` から `4` までの 5 本です。
