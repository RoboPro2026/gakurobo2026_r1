...
void test_trajectory_planner2()
{
  // ウェイポイントを設定
  std::vector<double> x_wp = {0.0, 3.0, 6.0, 9.0, 12.0};
...
void test_trajectory_planner3()
{
  double D = 5.0;
  double R = 5;
  double V = 5.0;
  // ウェイポイントを設定
  std::vector<double> x_wp, y_wp, theta_wp, v_trans_wp;
  x_wp.push_back(-D);
  y_wp.push_back(-R);
  theta_wp.push_back(0.0);
  v_trans_wp.push_back(0.0);

  auto theta = linspace(-M_PI / 2, 0, 6);
  for (int i = 0; i < static_cast<int>(theta.size()); i++) {
    RCLCPP_INFO(rclcpp::get_logger("trajectory_planner_test_node"), "theta[%d]=%lf", i, theta[i]);
    x_wp.push_back(R * cos(theta[i]));
    y_wp.push_back(R * sin(theta[i]));
    theta_wp.push_back(0.0);
    v_trans_wp.push_back(V);
  }
...
int main(int argc, char ** argv)
{
  ...
  // test_trajectory_planner1();
  // test_trajectory_planner2();
  test_trajectory_planner3();
