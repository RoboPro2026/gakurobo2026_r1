...
また、`/set_mecanum_yaw` を使って IMU の現在ヨー角に対するオフセットを更新でき、任意のヨー角を「前」とみなしてフィールド指向制御を行えます（`use_imu = true` のときのみ反映）。
...
- **Subscribe**
  ...
  - `/set_mecanum_yaw` (`std_msgs/msg/Float64`): フィールド指向制御で用いるヨー角 `theta` の基準を設定します。受け取った `data` [rad] を「現在のヨー角」とみなすよう、内部の `theta_offset` を `theta_offset = data - yaw_raw` に更新します（`use_imu = true` のときのみ有効）。
- **Publish**
...
`use_imu = true` の場合、内部で用いる角度は `theta = normalize(yaw_raw + theta_offset)` です。`theta_offset` は `/set_mecanum_yaw` を受け取ったときに `theta_offset = data - yaw_raw` として更新されます（`normalize` は `-pi`〜`pi` に正規化）。
...
- IMU ヨー角の基準をリセットする例（現在向きを `0 rad` とみなす）:  
  `ros2 topic pub --once /set_mecanum_yaw std_msgs/msg/Float64 "{data: 0.0}"`
