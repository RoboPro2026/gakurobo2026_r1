...
- 起動時に `zone` に応じた filebase から `<filebase><index>_robot_parameter.csv` と `<filebase><index>_waypoints.csv` を読み込み、各 ACT 用の軌道を生成する
...
- `/r1_machine_initialize` を受けたときは、追従中 ACT を `NONE` へ戻し、`TrajectoryFollower` の index・積分項・完了判定・実軌跡・目標姿勢を明示的にリセットして停止指令を出す
- `/chassis_act_ref` に `ACT_PAUSE` (1000) を受けたとき、現在の `act_step_` をポーズ前の状態として保存し、全 `TrajectoryFollower` の積分項をリセットして速度指令をゼロにし、`act_step_` を `ACT_PAUSE` に遷移する
- `/chassis_act_ref` に `ACT_RESUME` (1001) を受けたとき、保存しておいた `act_step_` を復元し、軌道追従を再開する（追従 index は保持されているため、中断位置から再開）
- `/chassis_tangent_pid_enable` を受けたとき、`TrajectoryFollower` の接線方向 PID 有効フラグを更新する
- `/r1_machine_initialize_done` を受けるまでは `/chassis_act_ref` と `/robot_move` を無視し、初期化完了後にだけ ACT 開始要求を受け付ける
...
- Subscribe
  ...
  - `/r1_machine_initialize` (`std_msgs/msg/Empty`): PS 初期化の開始通知。追従内部状態を明示リセットして停止する
  - `/r1_machine_initialize_done` (`std_msgs/msg/Empty`): PS 初期化完了通知。これを受けるまで ACT 要求を受け付けない
- `/chassis_tangent_pid_enable` (`std_msgs/msg/Bool`): 接線方向 PID の有効/無効。`false` を受けると接線方向 PID を無効化し、FF のみで進む。`true` で再有効化（積分値もリセット）
- Publish
  ...
  - `/robot_map_pos` (`geometry_msgs/msg/PoseStamped`): 現在の自己位置を `map` 座標系へ変換したデバッグ用姿勢
...
`/r1_machine_initialize` を受けたときは、`act_step_` を `NONE` に戻し、各 `TrajectoryFollower::reset()` を実行し、可視化用の目標姿勢と実軌跡も破棄したうえでゼロ速度を publish します。これにより、PS 初期化前の追従 index や PID 積分値を次回 ACT へ持ち越さないようにしています。
...
`/robot_move` を受け取ったときは、対象 ACT を軌道インデックスへ変換し、`forest_order` に含まれる番号に応じて軌道上の `v_trans` を `decel_speed` 以下へ落とす区間を上書きします。その後に軌道を再生成し、`act_step_` を更新します。現行実装では、内回りの減速判定は `ACT2`、外回りは `ACT4` に対して有効です。`kfs_mechanism_type` も未使用ではなく、`collect_kfs_offset` を適用する向きを決めるために参照します。新しく追加した `ACT3` は KFS 自動回収とは結び付けず、`3_*` の軌道を使います。

`/chassis_act_ref` に `ACTN` を直接 publish した場合は、`ACTN_START` と違って軌道追従器の reset は入りません。そのため通常の開始要求は `ACTN_START` か、必要なら `/robot_move` を使う前提です。初期化直後は `/r1_machine_initialize_done` を受けるまで `/chassis_act_ref` と `/robot_move` のどちらも無視します。

**ポーズ / レジューム**

`ACT_PAUSE` (1000) を受けると、現在の `act_step_` を内部に保存したうえで全 `TrajectoryFollower` の積分項をリセットし、速度指令をゼロにします。`act_step_` は `ACT_PAUSE` へ遷移します。ポーズ中は timer callback でゼロ速度を継続 publish します。追従 index (`idx_`) はリセットせず保持するため、`ACT_RESUME` (1001) を受けると保存済みの `act_step_` を復元し、中断した位置から軌道追従を再開します。

**接線方向 PID の動的無効化**

`/chassis_tangent_pid_enable` に `false` を受けると `TrajectoryFollower` の接線方向 PID を無効化します。この状態では接線方向は feedforward（目標速度 `v_ref_tangent`）のみで制御され、位置誤差による追従 PID 補正は入りません。法線方向・角度の PID は継続します。再び `true` を受けると接線方向 PID を有効化し、同時に積分値もリセットします。

可視化ループでは、必要に応じて現在の目標姿勢、速度矢印、ロボットマーカ、実軌跡を publish します。実軌跡は `odom` から `map` へ TF 変換した姿勢を蓄積しており、TF が取得できない場合はその周期の追加をスキップします。

## パラメータ

設定例は [`r1_machine_config.yaml`](../../r1_bringup/config/r1_machine_config.yaml) にあります。
...
| `act_filebase_red` | string | `""` | 赤ゾーン専用の軌道 CSV ベースパス。`zone == "red"` のとき必須です。 |
| `act_filebase_blue` | string | `""` | 青ゾーン専用の軌道 CSV ベースパス。`zone == "blue"` のとき必須です。blue 専用 CSV として waypoint はそのまま読みます。 |
| `zone` | string | `"red"` | 動作ゾーン。`"blue"` と `"red"` で別パラメータセットを読み込みます。waypoint はゾーン別 CSV をそのまま読みます。 |
...
| `red_inner_decel_center_pos.*` | `vector<double>` | `{-100.0, 100.0, 0.0}` | 赤ゾーン・内回り軌道で減速に使う長方形領域の中心 `[x, y, yaw]`。`zone == "red"` のとき有効。 |
| `blue_inner_decel_center_pos.*` | `vector<double>` | `{100.0, 100.0, 0.0}` | 青ゾーン・内回り軌道で減速に使う長方形領域の中心 `[x, y, yaw]`。`zone == "blue"` のとき有効。 |
| `red_outer_decel_center_pos.*` | `vector<double>` | `{-100.0, 100.0, 0.0}` | 赤ゾーン・外回り軌道で減速に使う長方形領域の中心 `[x, y, yaw]`。`zone == "red"` のとき有効。 |
| `blue_outer_decel_center_pos.*` | `vector<double>` | `{100.0, 100.0, 0.0}` | 青ゾーン・外回り軌道で減速に使う長方形領域の中心 `[x, y, yaw]`。`zone == "blue"` のとき有効。 |
| `decel_speed` | double | `0.0` | `robot_move` で指定された区間に適用する減速速度 [m/s]。 |
| `collect_kfs_offset` | double | `0.0` | `robot_move` の `kfs_mechanism_type` に応じて、減速矩形の中心へ加えるオフセット量 [m]。 |

注意:

- `act_filebase` は廃止されています。`act_filebase_red` / `act_filebase_blue` のうち、`zone` に対応する filebase を必ず指定します。
- `zone` に対応する filebase が空、または CSV を開けない場合は起動時に Fatal で停止します。
- `zone` は単体起動時はパラメータで与えますが、bringup では `r1_bringup.launch.py` 側から注入しています。
- 減速区間の中心座標はゾーン別パラメータ（`red_*` / `blue_*`）を直接指定します。座標変換はコード側では行いません。
- waypoint はゾーン別 CSV をそのまま読みます。青ゾーン用に調整済みの CSV を `act_filebase_blue` 配下へ置いてください。
- `collect_kfs_offset` の方向計算（`cos(rect_yaw)` / `sin(rect_yaw)`）は青ゾーンで未検証です。

## 入力ファイル

起動時に、実装で確保されている軌道本数ぶんのファイルを読み込みます。現行実装では `0` から `5` までの 6 本です。

- ロボットパラメータ: `<zone_act_filebase><index>_robot_parameter.csv`
- waypoint: `<zone_act_filebase><index>_waypoints.csv`

`zone_act_filebase` は `zone == "red"` なら `act_filebase_red`、`zone == "blue"` なら `act_filebase_blue` です。

例:

- `act_filebase_red := src/gakurobo2026_r1/data/red/`
  - `src/gakurobo2026_r1/data/red/0_robot_parameter.csv`
  - `src/gakurobo2026_r1/data/red/0_waypoints.csv`
  - `src/gakurobo2026_r1/data/red/1_robot_parameter.csv`
  - `src/gakurobo2026_r1/data/red/1_waypoints.csv`
  - `src/gakurobo2026_r1/data/red/2_robot_parameter.csv`
  - `src/gakurobo2026_r1/data/red/2_waypoints.csv`
  - `src/gakurobo2026_r1/data/red/3_robot_parameter.csv`
  - `src/gakurobo2026_r1/data/red/3_waypoints.csv`
  - `src/gakurobo2026_r1/data/red/4_robot_parameter.csv`
  - `src/gakurobo2026_r1/data/red/4_waypoints.csv`
  - `src/gakurobo2026_r1/data/red/5_robot_parameter.csv`
  - `src/gakurobo2026_r1/data/red/5_waypoints.csv`

ACT と軌道 index の対応は次のとおりです。
...
今回の番号追加に合わせて、軌道ファイルは次のように 1 つずつ後ろへ順送しています。
...
bringup では [`r1_bringup.launch.py`](../../r1_bringup/launch/r1_bringup.launch.py) から起動されます。
`robot_control_mode` は `r1_main_node` の初期 operation mode を切り替える引数で、既定値は `manual` です。
bringup では `cmd_vel_topic` を `/cmd_vel_target` に設定し、[`r1_chassis_velocity_control_node`](./r1_chassis_velocity_control_node.md) を経由して最終的な `/cmd_vel` になります。
...
- `ros2 topic echo /chassis_tangent_pid_enable`
