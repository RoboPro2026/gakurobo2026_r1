...
- PR には少なくとも次を含める
  ...
  - ユーザーや操縦者から見た挙動の変化。互換性のない変更があった場合は明記する
  - 再現手順と確認結果
- 挙動変更があるのに確認結果がない PR は、レビューしにくく事故要因になる
- Pull Request をマージするときは、通常の merge を使う
- `squash merge` などでコミット履歴を打ち消す行為は、トラブルの元になるため行わない

### コミット

- コミットメッセージは、少なくとも内容が一目で分かる日本語の件名にする
- ただし SSH でミニPCにアクセスしている関係上、デバッグのために雑なコミットメッセージで commit することは認める
- 例:
...
- やむを得ない `force push` は許容するが、注意して行うこと
