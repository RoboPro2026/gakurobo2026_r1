...
以下は一般的な絶対ルールというより、このリポジトリで揃えたい書き方です。
...
- 配列や `std::vector` を走査するカウンタ変数は `int` 型で統一する
- 参照とポインタのどちらでもよい場面では、参照ではなくポインタを使う
- クラスを受け渡すときなどの所有表現は、 `std::shared_ptr` で統一する
  - `std::unique_ptr` による厳密な所有権分離は、このリポジトリでは無理に使わなくてよい
...
private:
  ...
  void timer_callback()
  {
    ...
    msg.data = gain_ * count_;
    example_value_publisher_->publish(msg);

    RCLCPP_INFO(this->get_logger(), "count=%d value=%f", count_, msg.data);
    count_++;
  }
