...
`r1_laser_filter_node` は、`sensor_msgs/msg/LaserScan` の隣接 2 点から局所的な形状を計算し、不要な点を `inf` に置き換えたフィルタ済みスキャンを publish する ROS 2 ノードです。入出力トピック名はパラメータで変更できます。

## フィルタの意図

このフィルタは、隣接する 2 点が作る線分と、その線分の中点へ向かうベクトルの関係を見て、局所的に「面に沿って並んでいる」ような点を落とすことを意図しています。

実装では、`ranges[i]` と `ranges[i+1]` を XY 座標へ変換し、次の 2 つのベクトルを作ります。

- 隣接 2 点を結ぶ線分ベクトル
- 原点からその中点へ向かうベクトル

この 2 ベクトルの cosine 値を計算し、`|cos|` が大きい組を除外します。`|cos|` が大きいときは両ベクトルがほぼ平行または逆平行で、`|cos|` が小さいときは 90 度に近い関係です。このノードでは、90 度に近い変化を比較的「良い点」とみなし、0 度または 180 度に近い組を除外します。

そのため、壁面のように滑らかに並ぶ点列や、原点から見て接線方向に並ぶ点を抑え、角や局所的な形状変化を持つ点を残したい場合に向いています。一方で、一般的な障害物除去やノイズ除去を目的とした汎用フィルタではありません。
...
3. 各レンジを極座標から 2 次元座標 `(x, y)` に変換し、隣接 2 点の線分ベクトルと、その中点ベクトルを作ります。
4. `abs(cosine)` が `threshold` 以上なら、`ranges[i]` を `inf` に置き換えます。
5. 加工後の `LaserScan` を `filtered_scan_topic` に publish します。

この実装は隣接点の形状変化だけを見て単純に除外するもので、クラスタリング、壁面推定、外れ値推定は行いません。

## 現在の bringup との関係

`r1_bringup/launch/r1_slam.launch.py` では、現状このノードは起動対象に入っておらず、代わりに `laser_filters/scan_to_scan_filter_chain` を使う構成になっています。このノードを使う場合は、launch に追加するか、単体で起動してください。
