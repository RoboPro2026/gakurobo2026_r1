...
- waypoint 列（`x, y, theta, v_trans`）から 2D スプラインを構成
- 各セグメントの距離を計算
- 最大速度・加速度・躍度・角速度の制約下で台形加速（`AccelDesigner`）による速度プロファイル生成
- 5 次最小躍度軌道（`MinimumJerk`）による姿勢軌道生成
...
- 代表的なメソッド:
  - `std::vector<int> calc(...)`  
    waypoint と拘束条件を与えて軌道を計算し、各区間のステータス配列を返す（0: 正常、負値: 警告/失敗）。
  - `get_trajectory()`  
...
```bash
python src/gakurobo2026_r1/r1_control/example/test_trajectory_planner_plot.py
```
